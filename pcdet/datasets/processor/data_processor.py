...
class DataProcessor(object):
    ...
    def mask_points_and_boxes_outside_range(self, data_dict=None, config=None):
        ...
        # validation
        if data_dict.get('annos', {}).get('gt_boxes_lidar', None) is not None and config.REMOVE_OUTSIDE_BOXES and not self.training:
            mask = box_utils.mask_boxes_outside_range_numpy(
                data_dict['annos']['gt_boxes_lidar'], self.point_cloud_range, min_num_corners=config.get('min_num_corners', 1), 
                use_center_to_filter=config.get('USE_CENTER_TO_FILTER', True)
            )
